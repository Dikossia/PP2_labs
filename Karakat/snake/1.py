...
while running:
    ...
    pygame.display.flip()
    clock.tick(speed)
print("топ 3 игрока по счёту:")
cursor.execute("Select user_name,score From users JOIN user_score ON users.user_id=user_score.user_id ORDER BY score DESC LIMIT 3")
asc=cursor.fetchall()
for row in asc:
    print(f"Name: {row[0]},Счёт: {row[1]}")
